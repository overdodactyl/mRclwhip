---
title: A quick demonstration of ```mRclwhip::polish()```
output: rmarkdown::html_vignette
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(fig.width = 8, fig.height = 4, dpi = 600)
library(tidyverse)
library(officedown)
library(arsenal)
library(survival)
...
### Example using ```coxph()```
```{r}
coxph_res <- coxph(Surv(fu.time, fu.stat) ~ race + sex + bmi, data = mockstudy)
...
```{r}
mtcars <- mtcars %>%
  mutate_at(vars(gear, am, carb), as.character)
